...
function $Resolve(  $q,    $injector) {
  ...
  this.resolve = function (invocables, locals, parent, self) {
    ...
    locals = locals || {};
    parent = parent || extend($q.when({}), { $$promises: true });

    if (!isResolve(parent))
      throw new Error("blah blah $resolve.resolve() blah blah");

    // for test "does not call injectables overridden by a local"
    var resolves = omit(invocables, Object.keys(locals));
    var pathElement = new PathElement({ name: '___self', resolve: resolves });
    var emptyContext = new ResolveContext(new Path([]), new Path([]));

    function fakePathElement(name, resolvedValues) {
      var fakeResolves = {};
      forEach(resolvedValues, function(value, key) {
        return fakeResolves[key] = function() { return value };
      });
      var fakeState = { resolve: fakeResolves, name: name};
      return new PathElement(fakeState);
    }

    var outerPromise = parent.then(function(result) {
      var parentPE = fakePathElement('___parent', result);
      var localsPE = fakePathElement('___locals', locals);
      var fakeParentPath = new Path([ parentPE, localsPE ]);

      return fakeParentPath.resolve(emptyContext).then(function() {
        var context = new ResolveContext(fakeParentPath, new Path([ pathElement ]));
        var promise = pathElement.resolve(context);
        return promise.then(function() {
          var resolvables = context.getResolvableLocals('___self', { flatten: true });
          var unwrapped = map(resolvables, function(value) { return value.data; });
          extend(locals, unwrapped);
          outerPromise.$$promises = true;
          return locals;
        });
      });
    });
    outerPromise.$$promises = false;
    return outerPromise;
  };
  ...
  var ResolveContext = function(parentPath, currentPath) {
    ...
    function getResolvableLocals(stateName, options) {
      ...
      rejectCyclicDependencies(resolvables);
    ...
    function rejectCyclicDependencies(param) {
      // TODO: reject cyclic deps.
    }
